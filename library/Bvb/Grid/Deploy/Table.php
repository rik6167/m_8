...
class Bvb_Grid_Deploy_Table extends Bvb_Grid_DataGrid {
            ...
            if ($this->_failedValidation !== true) {
                ...
                foreach ( array_keys ( $final_values ) as $key ) {
                    ...
                    if ($mode == 'add') {
                        ...
                        if (isset ( $this->info ['add'] ['fields'] [$key] ['attributes'] ['extra_field'] )) {
                            unset ( $final_values [$key] );
                        }
                ...
                if ($mode == 'add' && is_array ( $final_values )) {
                    ...
                    try {
                        ...
                        if (null !== $this->_callbackBeforeInsert) {
                            call_user_func_array ( $this->_callbackBeforeInsert, $final );
                        }
                        $this->_db->insert ( $this->data ['table'], $final_values );

                        if (null !== $this->_callbackAfterInsert) {
                            call_user_func_array ( $this->_callbackAfterInsert, $final );
                        }
                        ...
                        //$this->message = $this->__ ( 'Error saving record' );
                ...
                if ($mode == 'edit' && is_array ( $final_values )) {
                    ...
                    try {
                        ...
                        if (null !== $this->_callbackBeforeUpdate) {
                            call_user_func_array ( $this->_callbackBeforeUpdate, $final );
                        }

                        $this->_db->update ( $this->data ['table'], $final_values, $queryUrl . $where );

                        if (null !== $this->_callbackAfterUpdate) {
                            call_user_func_array ( $this->_callbackAfterUpdate, $final );
                        }
                        ...
                        $this->messageOk = FALSE;
                        $this->message = $this->__ ( 'Error updating record' );
    ...
    function deleteRecord($sql) {
        ...
        try {
            ...
            $this->messageOk = FALSE;
            $this->message = $this->__ ( 'Error deleting record' );
    ...
    function formatField($campo, $valor, $options = array()) {
        ...
        switch ($tipo) {
            ...
            case 'enum' :
                ...
                break;
            case 'password':
                 $valor = "<input type=\"password\" $onchange id=\"filter_{$campo}\"   class=\"input_p\" value=\"" . $this->_filtersValues [$campo] . "\" $opt>";
                break;
    ...
    function buildFormElement($field, $inicial_value = '', $mod = 'edit', $fieldValue = '') {
        ...
        if (isset ( $options ['values'] )) {
            ...
            if (is_array ( $options ['values'] )) {
                ...
                $valor = "<select name=\"$fieldRaw\" id=\"{$fieldRaw}\"   $attr >";
        ...
        switch ($tipo) {
            ...
            case 'set' :
                ...
                $valor = "<select multiple=\"multiple\"  size=\"$size\" name=\"{$fieldRaw}[]\" id=\"{$fieldRaw}\" $attr  >";
                foreach ( $avalor as $value ) {
            ...
            case 'text' :
            case 'textarea' :
                $valor = "<textarea  name=\"{$fieldRaw}\" id=\"{$fieldRaw}\"   $attr>" . stripslashes ( $inicial_value ) . "</textarea>";
                break;
            case 'password' :
                $valor = "<input  type=\"password\"  name=\"{$fieldRaw}\" id=\"{$fieldRaw}\"   value=\"" . stripslashes ( $inicial_value ) . "\" $attr>";

                break;
            case 'date' :
                $valor = "<input  type=\"text\"  name=\"{$fieldRaw}\" id=\"{$fieldRaw}\"   value=\"" . stripslashes ( $inicial_value ) . "\" size=\"12\" maxlength=\"10\" class=\"datepicker\">";

                break;
            default :
                $valor = "<input  type=\"text\"  name=\"{$fieldRaw}\" id=\"{$fieldRaw}\"   value=\"" . stripslashes ( $inicial_value ) . "\" $attr>";
    ...
    function gridForm() {
        ...
        $form_hidden = " <input type=\"button\"  name=\"bvb_btncancel\"  onclick=\"window.location='$url'\" value=\"" . $this->__ ( 'Cancel' ) . "\"><input type=\"hidden\" name=\"_form_edit\" value=\"1\">";
        ...
        if (isset ( $final ['mode'] )) {
            ...
            if ($final ['mode'] == 'edit' && ! $this->_editNoForm) {
                ...
                $columns = $this->info ['edit'] ['fields'];
                foreach ( array_keys ( $columns ) as $key ) {
                    $fields [$key] = '';
                    if (isset ( $this->info ['edit'] ['fields'] [$key] ['attributes'] ['extra_field'] )) {
                        unset ( $columns [$key] );
                    }
                }
    ...
    function deploy() {
        ...
        if (((isset ( $this->ctrlParams ['edit'] ) && $this->ctrlParams ['edit'] == 1) || @$this->ctrlParams ['add'] == 1 || @$this->info ['double_tables'] == 1) || ($this->formPost == 1 && $this->formSuccess == 0)) {
            ...
            if (($this->allowAdd == 1 && $this->_editNoForm != 1) || ($this->allowEdit == 1 && strlen ( $this->_comm ) > 1)) {
                ...
                $grid .= "<form method=\"post\" name=\"bvb_form\" id=\"bvb_form\" action=\"\">" . $this->temp ['table']->formGlobal () . self::gridForm () . "</form><br><br>";
