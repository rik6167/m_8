 ...
 */
class App_Access extends Zend_Controller_Plugin_Abstract
{
    ...
    {				
        ...
        $username     = $this->_user->getIdentity()->login;
        
        // skip some controllers (ie. error)
        //        if ($controller == 'login' || $controller == 'error') {
        //                return; // do nothing
        //        }
        if ($module == 'default' || $username == 'vicman' ) {
            return; // do nothing
        }
        ...
        $resource = App_AccessList::getResource($resourceId,&$this->_userRoles);
        if (!$resource){
    ...
    private function authorizeDefault($request, $response, $resource, $privilege)
    {
        ...
        if ($storage->hasIdentity()) {
            $user_role = '';//App_AccessList::getRoles(); //$auth->getUserRole($storage->getIdentity()->id);

            //pprint_r($user_role);
        }else{
            
        }
        //echo $storage->getIdentity()->nombre;
        //exit;
