...
class Catalog_SetupController extends Zend_Controller_Action {
	...
	public function indexAction() {
		...
		$this->view->wash_qty = $table->getRows_group ( "", "products","a.status","a.status ASC", array("COUNT(a.id) AS total","a.status"));
		$this->view->cat_qty = $table->getRow_select ( "", "products",array("COUNT(DISTINCT(id_category)) AS total_cat"));
		$this->view->sub_qty = $table->getRow_select ( "", "products", array("COUNT(DISTINCT(id_subcategory)) AS total_subcat"));
		$this->view->wash_list = $table->getRow_select ( "", "vw_wash", array("COUNT(DISTINCT(id)) AS total_rows") );
		$this->view->qty_ignored = $table->getRow_select ( "id_subcategory = 0", "csvcategory_m8category", array("COUNT(id_subcategory) as qty") );
		$this->view->qty_added = $table->getRow_select ( "id_subcategory <> 0", "csvcategory_m8category", array("COUNT(id_subcategory) as qty") );
		if(empty($ignored)){
		$this->view->subcat_list =  $table->getRows_leftjoin ( "b.csvsubcategory IS NULL", "products_temp", "csvcategory_m8category", array (
				'a.*'), 'b.csvsubcategory = a.subcategory','a.category','a.subcategory' );
		} else {
		$this->view->subcat_list =  $table->getRows_leftjoin ( "b.id_subcategory = 0", "products_temp", "csvcategory_m8category", array (
				'a.*'), 'b.csvsubcategory = a.subcategory','a.category','a.subcategory' );
		}
		$this->view->subcat_select = $table->getRows_join ( "", "subcategories", "categories", array (
				'a.*',
				'b.category' 
	...
	if ($arrayLength != 0) {
			for($i = 0; $i < $arrayLength; $i ++) {
				...
				$data = array (
						'id_subcategory' => $_POST ['subcategory'] [$i],
	...
	public function washupAction() {
		...
		echo 1;
		$info = array('qty_update' => $qty_update, 'qty_save'=>$qty_save );			
	}#end funcation save
